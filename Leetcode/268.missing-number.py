...
# @lc code=start
class Solution:
    # Different way of doing full sum - partial sum = missing num
...
# Full array sum - missing number sum = missing number
class Solution:
    def missingNumber(self, nums: List[int]) -> int:
        full = [n for n in range(len(nums) + 1)]
        return sum(full) - sum(nums)
